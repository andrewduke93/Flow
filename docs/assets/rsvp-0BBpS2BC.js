import{r as w}from"./animations-D9Wo9mUM.js";class h{constructor(){this.listeners=new Set,this.STORAGE_KEY="titan_reader_prefs_v15_true_book",this.config=this.loadFromStorage()}static getInstance(){return h.instance||(h.instance=new h),h.instance}getDefaultConfig(){return{id:"user-default",themeMode:"System",fontFamily:"New York",fontSize:18,lineHeight:1.35,paragraphSpacing:10,motionBlurIntensity:.05,showReflections:!1,rsvpSpeed:200,hasCustomSpeed:!1,rsvpChunkSize:1,isRSVPContextEnabled:!0,rsvpColorHighlight:"#FF3B30"}}loadFromStorage(){try{const t=localStorage.getItem(this.STORAGE_KEY);if(t){const e=JSON.parse(t),s={...this.getDefaultConfig(),...e};return s.hasCustomSpeed||(s.rsvpSpeed=200),s}}catch(t){console.warn("[TitanSettings] Failed to load config",t)}return this.getDefaultConfig()}saveToStorage(){try{localStorage.setItem(this.STORAGE_KEY,JSON.stringify(this.config))}catch(t){console.warn("[TitanSettings] Failed to save config",t)}}getSettings(){return{...this.config}}updateSettings(t){this.config={...this.config,...t},this.saveToStorage(),this.notify()}subscribe(t){return this.listeners.add(t),()=>this.listeners.delete(t)}notify(){this.listeners.forEach(t=>t())}}const R=()=>{const l=h.getInstance(),[t,e]=w.useState(l.getSettings());return w.useEffect(()=>{const s=()=>e(l.getSettings());return l.subscribe(s)},[]),{settings:t,updateSettings:s=>l.updateSettings(s)}};class P{static pulse(t){if(typeof navigator>"u"||!navigator.vibrate)return;const e=t.punctuation||"";if(/[.?!]/.test(e)){navigator.vibrate(15);return}if(/[,;:]/.test(e)){navigator.vibrate(5);return}}static impactMedium(){typeof navigator<"u"&&navigator.vibrate&&navigator.vibrate(20)}static impactLight(){typeof navigator<"u"&&navigator.vibrate&&navigator.vibrate(5)}static selectionChanged(){typeof navigator<"u"&&navigator.vibrate&&navigator.vibrate(3)}}const m=new Set([".","!","?","…","‽"]),x=new Set([";",":","—","–"]);class f{constructor(){this.tokens=[],this.currentIndex=0,this.wpm=150,this.rampStep=3,this.sentenceBoostEnabled=!0,this.wordsInCurrentSentence=0,this._isPlaying=!1,this.animationFrameId=null,this.lastFrameTime=0,this.accumulatedTime=0,this.listeners=new Set,this.completionListeners=new Set,this.notifyScheduled=!1,this.loop=t=>{if(!this._isPlaying)return;if(this.lastFrameTime===0){this.lastFrameTime=t,this.animationFrameId=requestAnimationFrame(this.loop);return}const e=(t-this.lastFrameTime)/1e3;this.lastFrameTime=t;const s=Math.min(e,.1);this.accumulatedTime+=s;const i=this.currentToken;if(!i){this.finish();return}const n=60/this.wpm;let r=1;this.rampStep===0?r=2:this.rampStep===1?r=1.5:this.rampStep===2&&(r=1.2);let a=1;if(this.sentenceBoostEnabled&&i.originalText){const o=i.originalText,c=o.length>0?o.charAt(o.length-1):"",k=o.length>1?o.charAt(o.length-2):"",b=m.has(c)||c==='"'&&m.has(k)||c==="'"&&m.has(k)||c==="”"&&m.has(k)||c==="’"&&m.has(k),T=x.has(c);b?(a=1+.3*Math.max(1,this.wpm/200),this.wordsInCurrentSentence=0):T?a=1.15:this.wordsInCurrentSentence++}const p=n*i.durationMultiplier*r*a;this.accumulatedTime>=p&&(this.advance(),this.accumulatedTime-=p),this._isPlaying&&(this.animationFrameId=requestAnimationFrame(this.loop))}}static getInstance(){return f.instance||(f.instance=new f),f.instance}get currentToken(){return this.currentIndex>=0&&this.currentIndex<this.tokens.length?this.tokens[this.currentIndex]:null}get isPlaying(){return this._isPlaying}setTokens(t){this.tokens=t,this.currentIndex>=t.length&&(this.currentIndex=0),this.notify()}clear(){this.pause(),this.tokens=[],this.currentIndex=0,this.notify()}play(){this._isPlaying||this.tokens.length!==0&&(this.currentIndex>=this.tokens.length-1&&(this.currentIndex=0),this._isPlaying=!0,this.lastFrameTime=0,this.accumulatedTime=0,this.currentIndex===0&&(this.rampStep=0),this.loop(performance.now()),this.notify())}pause(){this._isPlaying&&(this._isPlaying=!1,this.animationFrameId!==null&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null),this.notify())}stop(){this.pause(),this.accumulatedTime=0,this.rampStep=3,this.animationFrameId&&(cancelAnimationFrame(this.animationFrameId),this.animationFrameId=null),this.notify()}toggle(){this._isPlaying?this.pause():this.play()}seek(t){const e=this._isPlaying;e&&this.pause(),this.currentIndex=Math.max(0,Math.min(t,this.tokens.length-1)),this.accumulatedTime=0,this.notify(),e&&this.play()}updateWPM(t){this.wpm=Math.max(50,Math.min(2e3,t)),this.notify()}advance(){this.currentIndex<this.tokens.length-1?(this.currentIndex++,this.rampStep<3&&this.rampStep++,this.notify()):this.finish()}finish(){this.pause(),this.currentIndex=this.tokens.length-1,this.notify(),this.completionListeners.forEach(t=>t())}subscribe(t){return this.listeners.add(t),()=>this.listeners.delete(t)}onComplete(t){return this.completionListeners.add(t),()=>this.completionListeners.delete(t)}notify(){this._isPlaying&&!this.notifyScheduled?(this.notifyScheduled=!0,queueMicrotask(()=>{this.notifyScheduled=!1,this.listeners.forEach(t=>t())})):this._isPlaying||this.listeners.forEach(t=>t())}}const I=`
